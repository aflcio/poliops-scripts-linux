...
def copy_check_files(config_path: str,
    ...
    def name_prospective_files(specs: dict[str, dict[str, str]],
        ...
        """ return a list of remote, local pairs. """
    ...
    def available_non_empty_files(client: SSHClient,
        ...
        for spec in prospective_files:
            logging.debug('checking for', spec.remote_path)
    ...
    def move_remote_files_aside(client: SSHClient,
        ...
        for spec in specs:
            command_line = f'cp {spec.remote_path} {spec.remote_path_old}'
    ...
    def copy_files_to_host(client: SSHClient,
                           file_specs: list[FileSpec]) -> list[str]:
        """
        ...
        for spec in file_specs:
            with open(spec.local_path, 'w') as ofh:
                ...
                rewrite_csv(host_out.stdout, ofh)
                retval.append(spec.local_path)

        return retval

    def copy_files_to_share(pathnames: list[str],
                            final_dir: str):
        for pathname in pathnames:
            company, filename = os.path.split(pathname)[-2]
            pathname2 = os.path.join(final_dir, company, filename)
            shutil.copyfile(pathname, pathname2)
    ...
    if len(files_wanted) > 0:
        pathnames = copy_files_to_host(client, files_wanted)
        print(pathnames)
